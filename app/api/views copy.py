    ...
    def get(self, request):
        ...
        for each in instance:
            ...
            try:
                ...
                data.update(obj)
        month_defaults = {'January': 0, 'February': 0, 'March': 0, 'April': 0, 'May': 0, 'June': 0,
                          'July': 0, 'August': 0, 'September': 0, 'October': 0, 'November': 0, 'December': 0}

        final = {}
        for year in data:
            months = ([*data[year]])
            default_months = month_defaults.copy()

            for m in months:
                if m in [*default_months]:
                    default_months.update({m: data[year][m]})
            final.update({year: default_months})
